...
> 把 AI 出图变成有人审节点、可追溯、可交接的游戏美术工作流。
...
首个实跑案例以雅典娜神话意象为起点，沿同一角色身份推进 7 个阶段：

`视觉调研 → 六方向探索 → 服装与身型衍生 → 角色定稿 → 建模/绑定视图 → 中性白模 → 3D 模型验收`

页面前部提供可直接复制的工作流提示。生成前先确认衍生方向、差异幅度与交付目标；执行中使用统一进度结构。当前案例已确认方向 4、Outfit C、Physique B、角色定稿与建模结构，并接入由 Tripo AI 生成的可交互 3D 模型。

## 证据边界

- 网站中的样图来自仓库已有提示词记录。
- “实测”表示提示词附有实际生成结果，不表示它在所有模型、素材和生产条件下都稳定。
- 雅典娜案例中的已完成阶段来自同一角色连续生成；未完阶段明确显示为等待审核，不用旧图库图片代替。
- 页面交付的是图像到建模参考的工作流；白模图不是实际拓扑、绑定网格或引擎资产。
